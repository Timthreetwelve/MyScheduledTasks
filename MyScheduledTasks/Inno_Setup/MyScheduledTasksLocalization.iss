...
en.ClearSettings=Do you want to remove the settings files and registry entries?%n%nSelect 'No' if you plan to reinstall the program.

ko.NotSelfContained=이렇게 하면 %1 버전 %2.%n%n이 버전의 표준 버전이 설치됩니다. 이 버전은 .NET 10 데스크톱 런타임을 기존에 설치해야 하며 x64 및 x86 시스템과 모두 호환됩니다.%n%n계속하기 전에 다른 모든 응용 프로그램을 닫는 것이 좋습니다.%n%n계속하려면 '다음'을 클릭하거나 '취소'를 클릭하여 설치를 종료하는 것이 좋습니다.
ko.SelfContainedx86=이렇게 하면 %1 버전의 자체 포함 x86 (32비트) 버전 %2.%n%n계속하기 전에 다른 모든 응용 프로그램을 닫는 것이 좋습니다.%n%n계속하려면 '다음'을 클릭하거나 '취소'를 클릭하여 설치를 종료합니다.
ko.SelfContainedx64=이렇게 하면 %1 버전의 자체 포함 x64 (64비트) 버전 %2.%n%n계속하기 전에 다른 모든 응용 프로그램을 닫는 것이 좋습니다.%n%n계속하려면 '다음'을 클릭하거나 '취소'를 클릭하여 설치를 종료합니다.
ko.ViewReadme=ReadMe 파일 보기
ko.AppIsRunning=가 실행 중입니다. 설치를 계속하려면 닫으세요.
ko.ClearSettings=설정 파일과 레지스트리 항목을 제거하시겠습니까? %n%n프로그램을 다시 설치하려면 '아니오'를 선택합니다.
...
nl.ClearSettings=Wilt u de instellingen en gegevensbestanden verwijderen?%n%nKies 'NEE' als u het programma opnieuw wilt installeren.
